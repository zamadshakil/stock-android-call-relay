  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      if (!supabaseUrl || supabaseUrl.includes("your-project")) {
        // Fallback for local demo preview if Supabase keys aren't configured yet
        setTimeout(() => {
    ...
    <div className={styles.authPage}>
      ...
      <div className={styles.authCard}>
        ...
        <p className={styles.switchAuth}>
          Already have an account? <Link href="/login" className={styles.switchLink}>Log in <ArrowRight size={14} className="inline-block" /></Link>
        </p>
      </div>
    </div>
  );
